(function () {
    ...
    function IntitutionFormDialogController($scope, $mdDialog, $log, $timeout, NgMap, Institution, InstitutionService, WilayaService, InstitutionClassService, Toast, Upload, API_ENDPOINT) {
        ...
        if (vm.institution) {
            ...
            vm.markers = [{
                id: 0,
                lat: vm.institution.position.lat,
                lng: vm.institution.position.lng
            }];
        }
        ...
        NgMap.getMap('new-institution-map').then(function (map) {
            ...
            vm.map.setCenter(new google.maps.LatLng(vm.institution.position.lat, vm.institution.position.lng));
        ...
        function loadCommunes() {
            if (vm.institution && vm.institution.wilaya.id) {
                return WilayaService.get({ wilayaId: vm.institution.wilaya.id }, function (data) {
        ...
        function onMapClicked(event) {
            ...
            vm.map.setCenter(new google.maps.LatLng(vm.institution.latitude, vm.institution.longitude));
        }
        ...
        function getCoordinates() {
            ...
            vm.geocoder.geocode({
                'address': vm.wilayas[vm.institution.wilaya.id - 1].name + ", " +
                vm.selectedWilaya.communes.find(function (commune) { return commune.id === vm.institution.commune.id }).name
